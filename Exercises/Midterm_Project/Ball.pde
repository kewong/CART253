...
class Ball {
  ...
  void update() {
    ...
    // CHANGED whenever the ball reaches the upper or lower half of the screen, the ball will either increase or decrease in velocity causing a wavelength type effect 
    if (y < height/2) {
  ...
  // CHANGED the ball will change color from black to white depending on whether its location is in the left or right half of the screen
  ...
  void reset() {
    ...
    // CHANGED the second counter will increase whenever the ball missed the paddle
    counter2.value++;

    // CHANGED if the second counter's score reaches 10, the game will be over and the player will lose
    if (counter2.value >= 10)
    {
      // CHANGED this will decide whether the game will end or not
      gameOver =true;
    }
  }
  ...
  void collide(Paddle paddle) {
    ...
    if (insideLeft && insideRight && insideTop && insideBottom) {
      ...
      //CHANGED every time the ball hits one of the paddles, the score will increase
      counter.value++;

      // CHANGED made the background reset to black every time the ball hits the paddles
