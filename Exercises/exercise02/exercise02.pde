...
void setup() {
  ...
  // CHANGED the screen size to 840 pixels by 680 pixels
  size(840, 680);
  setupPaddle();
  setupBall();
...
// CHANGED created new float to ensure that the lines reach the middle of the screen
void drawNewBackground() {
  for (int i = 0; i < width; i++) {
    float r = random(255);
    float randomHeight = random(height/2);
    stroke(r);
    line(i, 0, i, randomHeight);
  }
}
...
// updateBallColor() was added into the updateBall() function to update the color of the ball
void updateBall() {
  ballX += ballVX;
  ballY += ballVY;
...
// CHANGE constrained the ball size from decreasing past 3 pixels so that the ball never disappears
void handleBallHitPaddle() {
  if (ballOverlapsPaddle()) {
    ballY = paddleY - paddleHeight/2 - ballSize/2;
    ballVY = -ballVY;
    ballSize = constrain(ballSize-5,3,1000);
